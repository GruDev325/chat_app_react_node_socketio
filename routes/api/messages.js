...
router.put("/global", (req, res) => {
    GlobalMessage.findOneAndUpdate(
        {
...
// Post remove Message
router.post("/removeGlobalMessage", (req, res) => {
  GlobalMessage.deleteOne({ _id: req.body.id })
    .then((data) => {
      console.log(data);
      res.setHeader("Content-Type", "application/json");
      res.end(
      ...
      req.io.sockets.emit("removeMessage", req.body.id);
    })
    .catch((err) => {
      console.log(err);
      res.setHeader("Content-Type", "application/json");
